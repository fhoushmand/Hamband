...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < 1000; i++) {
    test->addEmployee(std::to_string(i));
    test->addProject(std::to_string(i));
  }
  ...
  int num_conflicting_write_methods = 3;
  ...
  while (calls[0].size() > calls[1].size() && read_calls != 0) {
    calls[(index % (nr_procs - 1)) + 1].push_back(std::string("4"));
    read_calls--;
    index++;
  }
  ...
  if (read_calls != 0) {
    for (int i = 0; i < nr_procs; i++)
      for (int j = 0; j < read_calls / nr_procs; j++)
        calls[i].push_back(std::string("4"));
