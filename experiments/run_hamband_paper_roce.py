#!/usr/bin/env python3
"""Run the SafarDB paper's Hamband matrix on the OCT RoCE hosts."""
...
BAD_LOG_PATTERNS = (
    ...
    re.compile(r"failed to (post|poll|set|open|query)", re.IGNORECASE),
...
CSV_PATH = RESULTS_DIR / "hamband_roce_paper_4m.csv"
LOG_DIR = RESULTS_DIR / "hamband_roce_paper_4m_logs"
...
rdma link show | grep -q "rxe0/1 state ACTIVE"
test "$(cat /sys/class/infiniband/rxe0/ports/1/gid_attrs/types/1)" = "RoCE v2"
"""
...
def validate_logs(outputs: list[str], replicas: int, run_name: str) -> None:
    ...
    digests = []
    for node_id, output in enumerate(outputs, start=1):
        if "all nodes finished" not in output:
...
def main() -> int:
    ...
    selected_percentages = (
        (args.percentage,) if args.percentage is not None else PERCENTAGES
    )
