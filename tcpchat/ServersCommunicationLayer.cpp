...
public:
	ServersCommunicationLayer(int id, std::vector<string> hosts, int* portNumbers, ReplicatedObject* obj)
...
ServersCommunicationLayer::ServersCommunicationLayer(int id, std::vector<string> hosts, int* ports, ReplicatedObject* obj) 
        throw (HostNotFoundException*, GetSocketException*, ConnectException*){
    this->id = id;
    this->hosts = hosts;
    this->ports = ports;
...
ServerConnection* ServersCommunicationLayer::getConnection(int remoteId) {
    ...
    if(connections.find(remoteId) == connections.end()) {
        ret = new ServerConnection(id, remoteId, NULL, hosts, ports, object);
