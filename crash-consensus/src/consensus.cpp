...
void RdmaConsensus::run() {
  ...
  auto shared_memory_addr =
      ...
      std::cout << "shared_memory_addr: " << uintptr_t(shared_memory_addr) << std::endl;

  // allocator for scratchpad + log -> it starts at shared_memory_addr
  overlay =
      std::make_unique<OverlayAllocator>(shared_memory_addr, allocated_size);
  // we pass allocator to scratchpad so it can create the scratchpad which contains several required slots
  scratchpad =
      std::make_unique<ScratchpadMemory>(ids, *overlay.get(), alignment);
  // the remaining space is allocated for the actual log
  ...
  //adding a new log for deposits
  //****************///
  //****************///
  //****************///
  //****************///
  cb->registerPD("hamsaz");
    cb->allocateBuffer("deposit-buf", dory::constants::MAX_ENTRY_SIZE, alignment);
    ...
    auto deposit_memory_addr =
        reinterpret_cast<uint8_t*>(cb->mr("deposit-mr").addr);

    LOGGER_INFO(logger, "deposit_memory_addr: 0x{:x}", uintptr_t(deposit_memory_addr));
    
    deposit_overlay =
        std::make_unique<OverlayAllocator>(deposit_memory_addr, dory::constants::MAX_ENTRY_SIZE);
    std::cout << "overlay created\n";
    deposit_memory =
        std::make_unique<HamsazMemory>(ids, *deposit_overlay.get(), alignment);
    auto [d_logmem_ok, d_logmem, d_logmem_size] = deposit_overlay->allocateRemaining(alignment);
    std::cout << "deposit memory created\n";

    LOGGER_INFO(logger, "Deposit log allocation for deposit... {}", d_logmem_ok ? "OK" : "FAILED");
    LOGGER_INFO(logger, "Deposit log deposit(address: 0x{:x}, size: {} bytes)",
                uintptr_t(d_logmem), d_logmem_size);

    auto d_log_offset = d_logmem - deposit_memory_addr;
    std::cout << "here\n";
    deposit_replication_log = std::make_unique<Log>(d_logmem, d_logmem_size);
    std::cout << "here2\n";
    ...
    std::cout << "here7\n";
    d_re_conn_ctx = std::make_unique<ConnectionContext>(
        *cb.get(), *ce_deposit.get(), cq_deposit, remote_ids, my_id);
    std::cout << "here8\n";
    deposit_sqw = std::make_unique<SequentialQuorumWaiter>(
      quorum::EntryWr, d_re_ctx->cc.remote_ids, Identifiers::maxID(remote_ids) + 1, 1);
      std::cout << "here9\n";
    deposit_majW = std::make_unique<FixedSizeMajorityOperation<SequentialQuorumWaiter,
                                                     WriteLogMajorityError>>(
      &d_re_ctx->cc, *deposit_sqw.get(), d_re_ctx->cc.remote_ids);
  ...
  if (am_I_leader.load()) {  // Leader (slow and fast-path)
    ...
    if (likely(fast_path)) {  // Fast-path
      ...
      if (freshest != nullptr) {
        ...
        auto [address, offset, size] = slot.location();
        // add slot offset to all the entries in "to_remote_memory" and copy it in dest
        std::transform(to_remote_memory.begin(), to_remote_memory.end(),
                       dest.begin(), bind2nd(std::plus<uintptr_t>(), offset));
        auto err = majW->write(address, size, dest, leader);
