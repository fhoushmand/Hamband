...
//     //   int array[]{0};
//       Call call(replication_log, offsets[id-1], NULL, static_cast<uint8_t*>(payload), 10);
...
//       ParsedCall pslot(iter[i].location());
...
//           commit_iter[i].next();
//           ParsedCall pslot(commit_iter[i].location());
...
//           else
//             std::cout << "no deps" << std::endl;
//           std::cout << std::endl;
//           // std::cout << "deps: " << static_cast<int>(pslot.dependencies()[0]) << ", " << static_cast<int>(pslot.dependencies()[1]) <<
//                     // ", " << static_cast<int>(pslot.dependencies()[2]) << std::endl;
