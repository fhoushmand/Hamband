...
struct RemoteConnection {
  ...
  RemoteConnection() {
    std::memset(&rci, 0, sizeof(rci));
  }

  RemoteConnection(uint16_t lid, uint32_t qpn, union ibv_gid const &gid,
                   enum ibv_mtu mtu, uintptr_t buf_addr, uint64_t buf_size,
                   uint32_t rkey) {
    rci.lid = lid;
    rci.qpn = qpn;
    rci.gid = gid;
    rci.mtu = static_cast<uint8_t>(mtu);
  ...
  std::string serialize() const {
    ...
    os << std::hex << rci.lid << ":" << rci.qpn << ":";
    for (auto byte : rci.gid.raw) {
      os << std::setw(2) << std::setfill('0') << static_cast<unsigned>(byte);
    }
    os << ":" << static_cast<unsigned>(rci.mtu) << ":" << rci.buf_addr << ":"
  ...
  static RemoteConnection fromStr(std::string const &str) {
    ...
    auto const valid_gid =
        gid.size() == sizeof(rci.gid.raw) * 2 &&
        std::all_of(gid.begin(), gid.end(), [](unsigned char c) {
          return std::isxdigit(c) != 0;
        });
    if (ss.fail() || !valid_gid || mtu == 0 || mtu > IBV_MTU_4096) {
      throw std::runtime_error("Invalid serialized RDMA connection data");
    }

    rci.lid = lid;
    rci.qpn = qpn;
    rci.mtu = static_cast<uint8_t>(mtu);
    for (size_t i = 0; i < sizeof(rci.gid.raw); ++i) {
      try {
        rci.gid.raw[i] = static_cast<uint8_t>(
            std::stoul(gid.substr(i * 2, 2), nullptr, 16));
      } catch (std::exception const &) {
        throw std::runtime_error("Invalid GID in RDMA connection data");
      }
    }
    rci.buf_addr = buf_addr;
    rci.buf_size = buf_size;
