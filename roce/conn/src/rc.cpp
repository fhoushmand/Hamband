...
void ReliableConnection::connect(RemoteConnection &rc) {
  ...
  conn_attr.path_mtu = static_cast<enum ibv_mtu>(std::min(
      static_cast<uint8_t>(cb.activeMTU()), rc.rci.mtu));
...
RemoteConnection ReliableConnection::remoteInfo() const {
  RemoteConnection rc(static_cast<uint16_t>(cb.lid()), uniq_qp->qp_num,
                      cb.gid(), cb.activeMTU(), mr.addr, mr.size, mr.rkey);
  return rc;
}
