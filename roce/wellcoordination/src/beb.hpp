...
bool ReliableBroadcast::broadcast(uint8_t *payload, size_t length,
  ...
  for (auto &[pid, rc] : ce->connections()) {
    ...
    if (!ok) {
      ...
      ok = rc.postSendSingle(
          ...
          static_cast<uint32_t>(size), rc.remoteBuf() + offset);
      if (!ok) {
        throw std::runtime_error("Failed to post broadcast RDMA write");
      }
    }
    outstanding_writes++;
    i++;
  ...
  if (static_cast<size_t>(completed) > outstanding_writes) {
    throw std::runtime_error("Unexpected broadcast completion count");
  }
...
int ReliableBroadcast::pollCQ(int number) {
  ...
  while (true) {
    ...
    if (num < 0) {
      throw std::runtime_error("Failed to poll the broadcast completion queue");
    }

    for (int i = 0; i < num; ++i) {
      if (entries[static_cast<size_t>(i)].status != IBV_WC_SUCCESS) {
        throw std::runtime_error(
            "Broadcast RDMA write failed: " +
            std::string(ibv_wc_status_str(
                entries[static_cast<size_t>(i)].status)));
      }
    }
