...
uint8_t ControlBlock::linkLayer() const { return resolved_port.linkLayer(); }

union ibv_gid const &ControlBlock::gid() const {
  return resolved_port.portGID();
}

uint8_t ControlBlock::gidIndex() const { return resolved_port.gidIndex(); }

enum ibv_mtu ControlBlock::activeMTU() const {
  return resolved_port.activeMTU();
}
