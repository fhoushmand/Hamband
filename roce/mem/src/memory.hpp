...
// ADDED by FARZIN
// memory for non-conflicting effects of the state (deposit in a bank account)
class HamsazMemory {
 public:
  HamsazMemory(std::vector<int> &ids, OverlayAllocator &overlay,
                   int alignment);
 ...
 private:
  HamsazMemory(std::vector<int> &ids, Memory const &mem);
  void setup();

  // Add more entries here
  void setupDepositSlot();
 ...
 private:
  ...
  // Add more entries here
  uint8_t *deposit_slot;
  ptrdiff_t deposit_slot_offset;
